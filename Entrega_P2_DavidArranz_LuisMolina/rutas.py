...
def mostrar_ediciones():
    ...
    # la carga de información la base de datos. En particular, debeis contar cuantos documentos
    ...
    festivales = list(mongo.db["festivales"].
                      find({},
                           {"anyo": 1,
                            ...
                            "concursantes": 1}).
                      sort("anyo", -1).
                      skip((pagina - 1) * elementos_por_pagina). #logica para saltarse los elementos de paginas anteriores,
                      limit(elementos_por_pagina))               # encontrada con IA
...
def mostrar_festival(anyo: int):
    ...
    # Devuelve un error 404 si no se encuentra presente ese anyo.
    # Como respuesta, renderiza el template "mostrar_actuaciones_edicion.html"
    festival = mongo.db["festivales"].find_one_or_404({"anyo": anyo})

    participaciones = []
    #construyo la lista de diccionarios con toda la informacion pertinente
    for i in festival["concursantes"]:
        participaciones.append({
            ...
            "url_youtube": i["url_youtube"]
        })
    # cambio el return de la plantilla porque no concuerda con el template
    return render_template("mostrar_actuaciones_edicion.html",anyo=festival["anyo"],pais_organizador=festival["pais"],
        ciudad=festival["ciudad"],participaciones=participaciones,pagina=1)
...
def generar_quiz():
    ...
    anyos = []
    cursor_anyos = mongo.db["festivales"].aggregate([
        {"$group": {"_id": "$anyo"}},
        {"$sort": {"_id": -1}}
    ])
    for i in cursor_anyos:
        anyos.append(i["_id"])

    paises = []
    cursor_paises = mongo.db["festivales"].aggregate([
        {"$unwind": "$concursantes"},
        {"$group": {"_id": "$concursantes.pais"}},
        {"$sort": {"_id": 1}}
    ])
    for j in cursor_paises:
        paises.append(j["_id"])
...
def mostrar_actuaciones_pais(id_pais: str):
    ...
    # AQUI VA VUESTRO CODIGO
    festival = mongo.db["festivales"].find_one_or_404({"concursantes.id_pais": id_pais})
    nombre_pais = None
    for i in festival["concursantes"]:
        if i["id_pais"] == id_pais:
            nombre_pais = i["pais"]
            break
    ...
    festivales_pagina = list(
        mongo.db["festivales"]
        .find({"concursantes.id_pais": id_pais})
        .sort("anyo", -1)
    ...
    participaciones = []
    for j in festivales_pagina:
        for i in j["concursantes"]:
            if i["id_pais"] == id_pais:
                participaciones.append({
                    "anyo": j["anyo"],
                    "pais_organizador": j["pais"],
                    "ciudad": j["ciudad"],
                    "artista": i["artista"],
                    "cancion": i["cancion"],
                    "resultado": i["resultado"],
