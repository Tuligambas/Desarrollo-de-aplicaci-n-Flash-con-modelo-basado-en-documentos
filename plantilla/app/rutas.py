...
def mostrar_festival(anyo: int):
    ...
    festivales = mongo.db["festivales"].find_one({"anyo": anyo})
    if festivales is None:
        abort(404)
    return render_template("mostrar_actuaciones_edicion.html", festivales=festivales)
...
def generar_quiz():
    ...
    anyos = sorted(mongo.db["festivales"].distinct("anyo"), reverse=True)
    paises = sorted(mongo.db["festivales"].distinct("concursantes.pais"))
...
def mostrar_actuaciones_pais(id_pais: str):
    ...
    nombre_pais = festival["concursantes"][0]["pais"]
    ...
    participaciones = []
    for f in festivales_pagina:
        for c in f["concursantes"]:
            if c["id_pais"] == id_pais:
                participaciones.append({
                    "anyo": f["anyo"],
                    "pais_organizador": f["pais"],
                    "ciudad": f["ciudad"],
                    "artista": c["artista"],
                    "cancion": c["cancion"],
